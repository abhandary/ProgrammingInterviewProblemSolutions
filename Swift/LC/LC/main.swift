...
//  LC
//
//  Created by Akshay Bhandary on 4/24/16.
...
class Solution {
    ...
    // MARK: 242. Valid Anagram. Given two strings s and t, write a function to determine if t is an anagram of s.
    func isAnagram(s: String, _ t: String) -> Bool {
        return s.characters.sort() == t.characters.sort();
       // return s.characters.sort {$0 <  $1} == t.characters.sort {$0 < $1}
    }
    ...
    func isPalindrome(x: Int) -> Bool {
        ...
        var digitsCount:Int = Int(log10(Double(xLow)) + 1)
        var msd:Int = Int(pow(10, Double(digitsCount)))
        for (var ix = 0; ix < digitsCount / 2; ix++) {
            if (xHigh / msd != xLow % 10) {
                return false;
            }
            xHigh = xHigh / msd;
            xLow = xLow / 10;
    ...
    func lengthOfLongestSubstring(s: String) -> Int {
        ...
        for (var ix = 0; ix < slen; ix++) {

            let index = s.startIndex.advancedBy(ix)
            if (lookup[s[index]] != nil && lookup[s[index]] >= lastUnique) {
                maxLen = max(maxLen, ix - lastUnique)
                lastUnique = lookup[s[index]]! + 1
            }
            lookup[s[index]] = ix
        }
        maxLen = max(maxLen, s.characters.count - lastUnique)
    ...
    func twoSum(nums: [Int], _ target: Int) -> [Int] {
        ...
        for var ix = 0; ix < nums.count; ix++ {
            let val = target - nums[ix]
            if  myDict[val] != nil && myDict[val] != ix {
                results.append(ix)
                results.append(myDict[val]!)
                return results;
            }
        }
        return results
...
let result = sol.containsDuplicate([1, 2, 3, 4]);
print("Contains Duplicate \(result)");

let result2 = sol.containsNearbyDuplicate([1, 2, 3, 4, 1], 4)
print("Contains Nearby Duplicate \(result2)");

let result3 = sol.isAnagram("ted", "det");
print("Is Anagram \(result3)");

let result4 = sol.isIsomorphic("eggl", "baaf")
print("Is isIsomorphic = \(result4)");

print("result = \(sol.getHint("1123", "0111"))")
