...
print("To Hex:\(soln.toHex(-1))");

let str = "abba"
print("Length of longest substrin for \(str) = \(soln.lengthOfLongestSubstring(str))");

let x = 1534236469;
print("Reversed \(x) = \(soln.reverse(x))");

print("Is palindrome \(0) = \(soln.isPalindrome(0))")

let s = "DCXXI"
print("Roman \(s)  to Int \(soln.romanToInt(s))");

let s1 = "ccc"
print("Longest palindrome = \(soln.longestPalindrome(s1))");

print("Int to roman = \(soln.intToRoman(3999))");

print("Three sum = \(soln.threeSum([-1, 0, 1, 2, -1, -4]))");

print("Generate Parenthesis = \(soln.generateParenthesis(3))");

print("Count and Say = \(soln.countAndSay(x: 6))");

print("Permutations = \(soln.permutations([1, 2, 3]))");

print("Group Anagrams = \(soln.groupAnagrams(input: ["eat", "tea", "tan", "ate", "nat", "bat"]))");

// print("Spiral Matrix = \(soln.spiralOrder([[1]]))");

print("Unique Paths = \(soln.uniquePaths(1, 2))");
