...
{
public:
    string printBinary(string s) {
        string integer = s.substr(0, s.find('.'));
        string decimal = s.substr(s.find('.'), s.size());
        int val = atoi(s.c_str());
        float decimalVal = atof(decimal.c_str());
        ...
        while (decimalVal > 0) {
            if (decimalResult.size() > 32) return "";
            if (decimalVal == 1) {
                decimalResult += decimalVal;
                break;
            }
            
            double r = decimalVal * 2;
            if (r >= 1) {
                decimalResult += "1";
                decimalVal = r - 1;
            } else {
    ...
    string convert(string input, int b1, int b2) {
        string result;
        unordered_map<char, int> lookup = {{'0', 0}};
        int sum = 0;
        for (int ix = 0; ix < input.size();ix++) {
            sum *= b1;
            sum += isnumber(input[ix])? input[ix] - '0': input[ix] - 'A' + 10;
        }
    ...
    bool isPalindrome(int number) {
        int numDigis = log10(number) + 1;
        int x = number, x_rem = number;
        int pow10 = pow(10, numDigis - 1);
        for(int ix = 0; ix < numDigis / 2; ix++) {
...
int main(int argc, const char * argv[]) {
    ...
    Cracking crack;
   // cout << "binary:" << crack.printBinary("67.12") << endl;
    
    cout << " x / y = " << crack.divide(15, 2) << endl;
    cout << " convert: " << crack.convert("AB", 16, 2) << endl;
    cout << " reverse: " << crack.reverse(134133532) << endl;
    cout << " isPalindrome " << crack.isPalindrome(422171234) << endl;
